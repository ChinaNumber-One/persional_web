  ...
  $('.nav-menu li a,.down').on('click', function (e) {
    ...
    if (anchor.attr('href').indexOf('#') === 0) {
      if ($(anchor.attr('href')).length) {
        $('html, body')
          .stop()
          .animate({ scrollTop: $(anchor.attr('href')).offset().top - 70 }, 750)
      }
      e.preventDefault()
    }
  })
    ...
    $.ajax({
      ...
      success: function (flag) {
        if (flag) {
          ...
          reset_form()
          chack.text('发送成功，请耐心等待回复～') // Message Send Text
          chack.removeClass('error')
          chack.addClass('send')
          $('#form-chack').fadeOut(3000)
        } else {
          $('#form-chack').fadeIn(400)
          reset_form()
          chack.text('糟糕，发生错误了呢～') // Message Error Text
